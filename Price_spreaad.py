...
def bond_price(face_value, coupon_rate, market_rate, maturity_date, settlement_date, spread=0, frequency=2):
    
    maturity_date = datetime.strptime(maturity_date, "%Y-%m-%d")
    settlement_date = datetime.strptime(settlement_date, "%Y-%m-%d")

    effective_yield = market_rate + spread
    #print(f"Effective Yield (Market Rate + Spread): {effective_yield}")
    ...
    discount_rate = (effective_yield / 100) / frequency
    
    discount_factors = [1 / (1 + discount_rate) ** (t * frequency) for t in tenors]
    
    discounted_cash_flows = [cf * df for cf, df in zip(cash_flows, discount_factors)]

    dirty_price = sum(discounted_cash_flows)
    ...
    return {
        "Effective Interest Rate": round(effective_yield,4),
        "Dirty Price": round(dirty_price, 4),
